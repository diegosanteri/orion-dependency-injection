...
const readMainDependencyFile = async (mainDependencyFile, namespace) => {    
    ...
    return readSections(json, namespace);    
}
...
const readExternals = async (externals) => {
    ...
    if(externals) {
        for(let i=0; i < externals.length; i++) {
            ...
            const path = 'node_modules/' + external + '/@dependency.json';
            concatDependencies(dependencies, await readMainDependencyFile(path, external));
        }
    }
...
const concatDependencies = (dependencies, dependenciesToConcat) => {
    return dependenciesToConcat.forEach(dependency => dependencies.push(dependency));
}
